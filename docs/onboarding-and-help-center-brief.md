...
- The five triggers (Settings "Back up now," the reminder banner, `⌘B`, `⌘K` "Create a backup," File → "Back up…") — all the same command; the success chip (size · file count · schema version); where the ZIP lands (`backups/<timestamp>.zip`) and that there is no in-app "reveal in Finder"; the 7-day reminder.
