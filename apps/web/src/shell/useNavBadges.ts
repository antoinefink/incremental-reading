 ...
 * round-trip). The counts refresh on mount and whenever a command-level undo
 * fires (`UNDO_EVENT`) — undoing a delete / mark-done / triage changes what is
 * due or in the inbox. Outside the desktop shell (browser / Vite-only) there is
 * no SQLite, so the hook returns an empty map and every badge hides — it never
 * invents a count. No domain logic lives here: the counts are computed main-side
 * (the queue/inbox repositories); this only awaits the typed IPC promises.
 */
 ...
 * on mount and on `UNDO_EVENT`. Returns an empty map outside the desktop shell so
 * every badge hides (no invented counts).
 */
