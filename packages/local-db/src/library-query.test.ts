 ...
 *  - the per-facet counts are over the UNFILTERED live universe.
 */
...
describe("LibraryQuery.browse", () => {
  ...
  it("computes per-type / per-priority / per-status counts over the unfiltered universe", () => {
    buildUniverse();
    // Counts hold even when a narrowing filter is applied (they are over the universe).
