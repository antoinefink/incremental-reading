...
**Battletest everything.** Tests are part of the work, not an afterthought: every meaningful
change — new features, bug fixes, and polish alike — ships with the tests needed to prove it
correct and keep it correct, including edge cases and regressions. When you touch existing code,
add or strengthen its tests too. Every meaningful feature should include tests at the right level.
...
independent work, and run adversarial review/verification passes (e.g. understand → build →
review) so the output is more thorough and correct. Match the orchestration to the task; skip it
for trivial edits.
